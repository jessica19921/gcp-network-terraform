...
# hub2
/*
resource "google_compute_router" "hub2_eu_vpn_cr" {
  project = var.project_id_hub
  name    = "${local.hub2_prefix}eu-vpn-cr"
  network = google_compute_network.hub2_vpc.self_link
  region  = local.hub2_eu_region
  bgp {
    asn               = local.hub2_eu_vpn_cr_asn
    advertise_mode    = "CUSTOM"
    advertised_groups = null
  }
}

resource "google_compute_router" "hub2_us_vpn_cr" {
  project = var.project_id_hub
  name    = "${local.hub2_prefix}us-vpn-cr"
  network = google_compute_network.hub2_vpc.self_link
  region  = local.hub2_us_region
  bgp {
    asn               = local.hub2_us_vpn_cr_asn
    advertise_mode    = "CUSTOM"
    advertised_groups = null
  }
}*/
...
# hub2
/*
resource "google_compute_ha_vpn_gateway" "hub2_eu_gw" {
  project = var.project_id_hub
  name    = "${local.hub2_prefix}eu-gw"
  network = google_compute_network.hub2_vpc.self_link
  region  = local.hub2_eu_region
}

resource "google_compute_ha_vpn_gateway" "hub2_us_gw" {
  project = var.project_id_hub
  name    = "${local.hub2_prefix}us-gw"
  network = google_compute_network.hub2_vpc.self_link
  region  = local.hub2_us_region
}*/
...
module "site1_to_hub2_eu" {
  source             = "github.com/terraform-google-modules/cloud-foundation-fabric//modules/net-vpn-ha"
  project_id         = var.project_id_onprem
  region             = local.site1_region
  network            = google_compute_network.site1_vpc.self_link
  name               = "${local.site1_prefix}to-hub2-eu"
  vpn_gateway        = google_compute_ha_vpn_gateway.site1_gw.self_link
  peer_gcp_gateway   = google_compute_ha_vpn_gateway.hub2_eu_gw.self_link
  vpn_gateway_create = false
  router_create      = false
  router_name        = google_compute_router.site1_vpn_cr.name
...
module "hub2_eu_to_site1" {
  source             = "github.com/terraform-google-modules/cloud-foundation-fabric//modules/net-vpn-ha"
  project_id         = var.project_id_hub
  region             = local.hub2_eu_region
  network            = google_compute_network.hub2_vpc.self_link
  name               = "${local.hub2_prefix}eu-to-site1"
  vpn_gateway        = google_compute_ha_vpn_gateway.hub2_eu_gw.self_link
  peer_gcp_gateway   = google_compute_ha_vpn_gateway.site1_gw.self_link
  vpn_gateway_create = false
  router_create      = false
  router_name        = google_compute_router.hub2_eu_vpn_cr.name
  ...
  tunnels = {
    tun-0 = {
      ...
      bgp_peer_options = {
        advertise_groups    = null
        advertise_mode      = "CUSTOM"
        advertise_ip_ranges = local.advertised_prefixes.hub2_to_sites
        route_priority      = 100
      }
      ...
      vpn_gateway_interface           = 0
      peer_external_gateway_interface = null
      router                          = google_compute_router.hub2_eu_vpn_cr.name
      shared_secret                   = local.psk
    }
    tun-1 = {
      bgp_peer = {
...
module "site2_to_hub2_us" {
  source             = "github.com/terraform-google-modules/cloud-foundation-fabric//modules/net-vpn-ha"
  project_id         = var.project_id_onprem
  region             = local.site2_region
  network            = google_compute_network.site2_vpc.self_link
  name               = "${local.site2_prefix}to-hub2-us"
  vpn_gateway        = google_compute_ha_vpn_gateway.site2_gw.self_link
  peer_gcp_gateway   = google_compute_ha_vpn_gateway.hub2_us_gw.self_link
  vpn_gateway_create = false
  router_create      = false
  router_name        = google_compute_router.site2_vpn_cr.name
  ...
  tunnels = {
    tun-0 = {
      bgp_peer = {
        address = cidrhost(var.bgp_range.cidr7, 2)
        asn     = local.hub2_us_vpn_cr_asn
      }
      bgp_peer_options = {
        advertise_groups    = null
      ...
      bgp_session_range               = "${cidrhost(var.bgp_range.cidr7, 1)}/30"
      ike_version                     = 2
      vpn_gateway_interface           = 0
      peer_external_gateway_interface = null
      router                          = google_compute_router.site2_vpn_cr.name
    ...
    tun-1 = {
      bgp_peer = {
        address = cidrhost(var.bgp_range.cidr8, 2)
        asn     = local.hub2_us_vpn_cr_asn
      }
      bgp_peer_options = {
        advertise_groups    = null
      ...
      bgp_session_range               = "${cidrhost(var.bgp_range.cidr8, 1)}/30"
      ike_version                     = 2
      vpn_gateway_interface           = 1
      peer_external_gateway_interface = null
      router                          = google_compute_router.site2_vpn_cr.name
...
module "hub2_us_to_site2" {
  source             = "github.com/terraform-google-modules/cloud-foundation-fabric//modules/net-vpn-ha"
  project_id         = var.project_id_hub
  region             = local.hub2_us_region
  network            = google_compute_network.hub2_vpc.self_link
  name               = "${local.hub2_prefix}us-to-site2"
  vpn_gateway        = google_compute_ha_vpn_gateway.hub2_us_gw.self_link
  peer_gcp_gateway   = google_compute_ha_vpn_gateway.site2_gw.self_link
  vpn_gateway_create = false
  router_create      = false
  router_name        = google_compute_router.hub2_us_vpn_cr.name

  tunnels = {
    tun-0 = {
      bgp_peer = {
        address = cidrhost(var.bgp_range.cidr7, 1)
        asn     = local.site2_asn
      }
      bgp_peer_options = {
        advertise_groups    = null
      ...
      bgp_session_range               = "${cidrhost(var.bgp_range.cidr7, 2)}/30"
      ike_version                     = 2
      vpn_gateway_interface           = 0
      peer_external_gateway_interface = null
      router                          = google_compute_router.hub2_us_vpn_cr.name
      shared_secret                   = local.psk
    }
    tun-1 = {
      bgp_peer = {
        address = cidrhost(var.bgp_range.cidr8, 1)
        asn     = local.site2_asn
      }
      bgp_peer_options = {
        advertise_groups    = null
      ...
      bgp_session_range               = "${cidrhost(var.bgp_range.cidr8, 2)}/30"
      ike_version                     = 2
      vpn_gateway_interface           = 1
      peer_external_gateway_interface = null
      router                          = google_compute_router.hub2_us_vpn_cr.name
