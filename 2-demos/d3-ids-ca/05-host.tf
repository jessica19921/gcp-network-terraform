...
--host-project=${var.project_id_host}
EOT
}
...
resource "google_compute_global_address" "spoke1_psa_range1" {
  project       = var.project_id_host
  name          = "${local.spoke1_prefix}psa-range1"
  network       = google_compute_network.spoke1_vpc.self_link
  purpose       = "VPC_PEERING"
  address_type  = "INTERNAL"
  address       = split("/", local.spoke1_psa_range1).0
  prefix_length = split("/", local.spoke1_psa_range1).1
}

resource "google_compute_global_address" "spoke1_psa_range2" {
  project       = var.project_id_host
  name          = "${local.spoke1_prefix}psa-range2"
  network       = google_compute_network.spoke1_vpc.self_link
  purpose       = "VPC_PEERING"
  address_type  = "INTERNAL"
  address       = split("/", local.spoke1_psa_range2).0
  prefix_length = split("/", local.spoke1_psa_range2).1
}

# service networking connection

resource "google_service_networking_connection" "spoke1" {
  provider = google-beta
  network  = google_compute_network.spoke1_vpc.self_link
  service  = "servicenetworking.googleapis.com"

  reserved_peering_ranges = [
    google_compute_global_address.spoke1_psa_range1.name,
    google_compute_global_address.spoke1_psa_range2.name
  ]
}
