...
locals {
  spoke1_dns_rp_rules_local = {
    ...
    ("${local.spoke1_prefix}dns-rp-rule-us-psc-https-ctrl") = {
      dns_name   = "${local.spoke1_us_psc_https_ctrl_dns}."
      local_data = "name=${local.spoke1_us_psc_https_ctrl_dns}.,type=A,ttl=300,rrdatas=${local.spoke1_us_ilb7_https_addr}"
    }
...
module "spoke1_dns_private_zone" {
  ...
  recordsets = {
    ...
    "A ${local.spoke1_us_ilb7_https_dns}" = { type = "A", ttl = 300, records = [local.spoke1_us_ilb7_https_addr] },
  }
}
...
locals {
  spoke1_eu_ilb7_domains = [
    ...
    local.spoke1_eu_psc_https_ctrl_dns
  ]
}
...
locals {
  spoke1_eu_ilb7_psc_neg_name      = "${local.spoke1_prefix}eu-ilb7-psc-neg"
  spoke1_eu_ilb7_psc_neg_self_link = "projects/${var.project_id_spoke1}/regions/${local.spoke1_eu_region}/networkEndpointGroups/${local.spoke1_eu_ilb7_psc_neg_name}"
  spoke1_eu_ilb7_psc_neg_create = templatefile("scripts/neg/psc7/create.sh", {
    PROJECT_ID     = var.project_id_spoke1
    NETWORK        = google_compute_network.spoke1_vpc.self_link
    REGION         = local.spoke1_eu_region
    NEG_NAME       = local.spoke1_eu_ilb7_psc_neg_name
    TARGET_SERVICE = local.spoke1_eu_psc_https_ctrl_dns
  })
  spoke1_eu_ilb7_psc_neg_delete = templatefile("scripts/neg/psc7/delete.sh", {
    PROJECT_ID = var.project_id_spoke1
    REGION     = local.spoke1_eu_region
    NEG_NAME   = local.spoke1_eu_ilb7_psc_neg_name
  })
...
resource "null_resource" "spoke1_eu_ilb7_psc_neg" {
  triggers = {
    create = local.spoke1_eu_ilb7_psc_neg_create
    delete = local.spoke1_eu_ilb7_psc_neg_delete
  }
  provisioner "local-exec" {
    command = self.triggers.create
  }
...
locals {
  ...
  spoke1_eu_ilb7_backend_services_psc_neg = {
    ("psc7") = {
      ...
      backends = [
        {
          group           = local.spoke1_eu_ilb7_psc_neg_self_link
...
module "spoke1_eu_ilb7_bes" {
  depends_on               = [null_resource.spoke1_eu_ilb7_psc_neg]
...
resource "google_compute_region_url_map" "spoke1_eu_ilb7_url_map" {
  ...
  host_rule {
    path_matcher = "psc7"
    hosts        = [local.spoke1_eu_psc_https_ctrl_dns]
  }
  path_matcher {
    name            = "main"
  ...
  path_matcher {
    name            = "psc7"
    default_service = module.spoke1_eu_ilb7_bes.backend_service_psc_neg["psc7"].self_link
  }
}
...
module "spoke1_eu_ilb7_frontend" {
  ...
  frontend = {
    ...
    https = {
      enable   = true
      address  = local.spoke1_eu_ilb7_https_addr
      port     = 443
