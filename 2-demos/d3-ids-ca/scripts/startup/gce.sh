...
echo  "\$(curl -k --max-time 1.5 -H 'Cache-Control: no-cache' -w "%%{http_code} (%%{time_total}s) - %%{remote_ip}" -s -o /dev/null ${target}) - ${target}"
%{ endfor ~}
echo -e "\n psc ...\n"
...
%{ endfor ~}
echo -e "\n td ...\n"
%{ for target in SCRIPTS.targets_td ~}
echo  "\$(curl -k --max-time 1.5 -H 'Cache-Control: no-cache' -w "%%{http_code} (%%{time_total}s) - %%{remote_ip}" -s -o /dev/null ${target}) - ${target}"
%{ endfor ~}
echo ""
EOF
...
i=0
while [ \$i -lt 3 ]; do
  %{ for t in SCRIPTS.targets_probe ~}
  ab -n \$1 -c \$2 ${t}/ > /dev/null 2>&1
  %{ endfor ~}
  let i=i+1
  sleep 3
done
...
*/3 * * * * /usr/local/bin/probez 5 1 2>&1 > /dev/null
*/5 * * * * /usr/local/bin/probez 4 4 2>&1 > /dev/null
EOF
